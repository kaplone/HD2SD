...
class Merge():
    ...
    def suite(self, chemin_in, nom):
        m2 = subprocess.Popen("ffmpeg -y -i '%s' -s 720x576 -sws_flags lanczos -pix_fmt yuv420p -b:a 384k -vcodec mpeg2video -q:v 0 /mnt/2015_rushs/4557/%s.mpg" % (chemin_in, nom), shell=True, stdout = subprocess.PIPE, bufsize = 1 , universal_newlines = True)            
        while m2.poll() == None:
            time.sleep(0.1)
     #   os.remove(chemin_in)
...
liste = Liste()
merge.merger(liste.new_liste("/mnt/2015_cartes/4000-5000/4557"))
