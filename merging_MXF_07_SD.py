...
class Merge():
    ...
    def merger(self, liste):
        ...
        liste_transmise,liste_ordonnee, numero, sous_chemin, racine = liste
        ...
        for l in liste_ordonnee:
            
            self_m2t_temp1 = "/".join((self.temp, l + ".mpg"))
            
            #mt = subprocess.Popen("ffmpeg -y -i '%s' -an -vcodec mpeg2video -q:v 0 %s" % (liste_transmise[l], self_m2t_temp1), shell=True)#, stdout = subprocess.PIPE, bufsize = 1 , universal_newlines = True)           
            mt = subprocess.Popen("ffmpeg -y -i '%s'  -vf 'yadif=0:0:0' -an  -s 720x576 -sws_flags lanczos -pix_fmt yuv420p -vcodec mpeg2video -q:v 0 %s" % (liste_transmise[l], self_m2t_temp1), shell=True)#, stdout = subprocess.PIPE, bufsize = 1 , universal_newlines = True)           
            while mt.poll() == None:
                time.sleep(0.1)
    ...
    def suite(self, chemin_in, a0, a1, nom):
        ...
        m2 = subprocess.Popen("ffmpeg -y -i '%s' -i '%s' -i '%s' -filter_complex '[1:0][2:0] amerge=inputs=2' -b:a 384k -ac 2 -vcodec mpeg2video -q:v 0 '%s/%s.mpg'" % (chemin_in, a0, a1, self.out, nom), shell=True, stdout = subprocess.PIPE, bufsize = 1 , universal_newlines = True)            
        while m2.poll() == None:
            time.sleep(0.1)
...
class Liste():
    ...
    def new_liste(self, start_path):
        ...
        liste_ordonnee = self.sorted_ls("/".join((start_path, "CONTENTS", "VIDEO")))
        print(liste_ordonnee)
        return (liste_des_videos, liste_ordonnee, self.numero, self.sous_chemin, self.racine)
