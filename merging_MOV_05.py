...
class Merge():
    ...
    def suite(self, chemin_in, nom):
        m2 = subprocess.Popen("ffmpeg -y -i '%s' -acodec copy -vcodec mpeg2video -q:v 0 /mnt/rushs_OUT_2/3474_1/%s.m2t" % (chemin_in, nom), shell=True, stdout = subprocess.PIPE, bufsize = 1 , universal_newlines = True)            
        while m2.poll() == None:
            time.sleep(0.1)
        os.remove(chemin_in)
            
    def suiteDeint(self, chemin_in, nom):
		m2 = subprocess.Popen("ffmpeg -y -i '%s'  -vf 'yadif=0:0:0' -acodec copy -vcodec mpeg2video -q:v 0 /mnt/rushs_OUT_2/3474_1/%s.m2t" % (chemin_in, nom), shell=True, stdout = subprocess.PIPE, bufsize = 1 , universal_newlines = True)            
        while m2.poll() == None:
            time.sleep(0.1)
        os.remove(chemin_in)
