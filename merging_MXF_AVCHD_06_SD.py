...
class Merge():
    ...
    def merger(self, liste):
        ...
        for l in sorted(liste_transmise.keys()):
            ...
            if self.dernier:
                #self.suiteDeint(self.outpath, self.precedent)
    ...
    def suite(self, chemin_in, nom):
        ...
        m2 = subprocess.Popen("ffmpeg -y -i '%s' -s 720x576 -sws_flags lanczos -pix_fmt yuv420p -b:a 384k -acodec ac3 -b:a 256k -ac 2 -vcodec mpeg2video -q:v 0 '%s/%s.m2t'" % (chemin_in, self.out, nom), shell=True, stdout = subprocess.PIPE, bufsize = 1 , universal_newlines = True)            
        while m2.poll() == None:
            time.sleep(0.1)
                
    def suiteDeint(self, chemin_in, nom):
        
        m2 = subprocess.Popen("ffmpeg -y -i '%s' -vf 'yadif=0:0:0' -s 720x576 -sws_flags lanczos -pix_fmt yuv420p -b:a 384k -acodec ac3 -b:a 256k -ac 2 -vcodec mpeg2video -q:v 0 '%s/%s.m2t'" % (chemin_in, self.out, nom), shell=True, stdout = subprocess.PIPE, bufsize = 1 , universal_newlines = True)            
        while m2.poll() == None:
            time.sleep(0.1)
