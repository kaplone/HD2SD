...
class Merge():
    ...
    def merger(self, liste):
        ...
        for l in liste_ordonnee:
            ...
            #mt = subprocess.Popen("ffmpeg -y -i '%s'  -vf 'yadif=0:0:0' -an -vcodec mpeg2video -q:v 0 %s" % (liste_transmise[l], self_m2t_temp1), shell=True)#, stdout = subprocess.PIPE, bufsize = 1 , universal_newlines = True)           
            while mt.poll() == None:
                time.sleep(0.1)
    ...
    def suite(self, chemin_in, a0, a1, nom):
        ...
        m2 = subprocess.Popen("ffmpeg -y -i '%s' -i '%s' -i '%s' -filter_complex '[1:0][2:0] amerge=inputs=2' -acodec ac3 -b:a 256k -ac 2 -vcodec mpeg2video -q:v 0 '%s/%s.m2t'" % (chemin_in, a0, a1, self.out, nom), shell=True, stdout = subprocess.PIPE, bufsize = 1 , universal_newlines = True)            
        while m2.poll() == None:
            time.sleep(0.1)
